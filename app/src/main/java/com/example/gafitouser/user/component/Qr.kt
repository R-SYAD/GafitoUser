...
fun QrCodeImage(content: String, size: Dp = 200.dp) {
    val qrPainter = rememberQrBitmapPainter(content = content, size = size)
    androidx.compose.foundation.Image(painter = qrPainter, contentDescription = "QR Code")
}
...
fun QrPreview() {
    MaterialTheme {
        QrCodeImage(content = "https://www.example.com", size = 200.dp)
    }
}
